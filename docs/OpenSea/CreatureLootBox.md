...
> CreatureLootBox CreatureLootBox - a tradeable loot box of Creatures.
...
```solidity
function itemsPerLootbox() external view returns (uint256)
```
...
```solidity
function unpack(uint256 _tokenId) external nonpayable
```
